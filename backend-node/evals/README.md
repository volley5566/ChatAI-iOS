...
> 有了它,改完跑一次 `npm run eval` 看分数升降。

---

## Eval 和主流程的关系(最重要的一节,先看这个)

**Eval 不在主请求流程里**——它是一个完全独立的离线工具。

用最通俗的比喻:主流程是"餐厅营业",Eval 是"卫生局来检查"。
餐厅正常营业时不会有人做检查;检查是另外的人、在另外的时间做的,
但检查的对象就是这个餐厅本身(同一套厨房、同一个厨师)。
...
│     → Express 路由 → Agent(LangGraph)→ MCP 工具 → DeepSeek            │
│     → SSE 流式回答 → iOS 气泡显示                                      │
│                                                                         │
│   这条路径 24 小时跑着,每个用户请求走一次。                              │
│   Eval 完全不参与这条路径。                                              │
└─────────────────────────────────────────────────────────────────────────┘

                          ↕ 共用同一个 Agent 代码(agentGraph.ts / agentRunner.ts)

┌─────────────────────────────────────────────────────────────────────────┐
│                         Eval 流程(离线 / 手动触发)                      │
│                                                                         │
│   开发者在终端执行 npm run eval                                         │
│     → 读 qa.jsonl(21 道题)                                            │
│     → 逐条调 runAgent()——绕过 HTTP,直接调 Agent 核心逻辑              │
│     → 4 个 evaluator 打分                                               │
│     → 终端输出成绩单                                                    │
│                                                                         │
│   这条路径只在开发者主动跑时才执行,不影响线上用户。                       │
└─────────────────────────────────────────────────────────────────────────┘
```
...
| 场景 | 怎么跑 | 目的 |
|---|---|---|
| **改了 prompt** | `npm run eval` | 看改完分数升了还是降了 |
| **改了工具逻辑**(如 searchKnowledge) | `npm run eval` | 确认工具行为没被改坏 |
| **升级了模型**(如 DeepSeek V3 → V4) | `npm run eval` | 新模型是否依然表现好 |
| **提交 PR 前快速检查** | `npm run eval -- --quick` | 跑前 5 条,30 秒出结果 |
| **CI 自动化**(Phase 10.3 引入) | GitHub Actions 里跑 | PR 自动检查,分数低了不让合并 |
| **日常开发不改 Agent** | **不用跑** | Eval 只测 Agent,改 UI / 改数据库不需要 |

### Eval 和主流程共享了什么?

```
evals/lib/runAgent.ts
  │
  │  import
  ▼
src/agent/agentRunner.ts  ←── 灰度入口(USE_LANGGRAPH 切 Phase 3/4)
  │
  ▼
src/langchain/agentGraph.ts (或 agentRunner.ts)
  │
  ▼
同一个 StateGraph / 同一套 MCP 工具 / 同一个 DeepSeek 模型 / 同一个 prompt
```

**关键设计**:`runAgent()` 调用的是和线上**完全相同**的 Agent 代码。
不是"模拟"也不是"简化版"——就是那个真正的 Agent,
只是跳过了 Express 路由和 SSE 协议包装,直接拿结果。

这就像 Android 的 Instrumented Test:跑的是真实的 ViewModel 和 Repository,
只是不启动 Activity UI。所以测试结果能代表真实行为。
...
**目前状态**:Phase 10.2 #5~#9 全部完成,所有文件已就绪,可以通过 `npm run eval` 运行。
